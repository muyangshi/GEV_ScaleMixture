...
# Reads in posterior draw of GEV from Stage 1
# Use those as proposals in Stage 2
# Require:
#   - utilities.py
if __name__ == "__main__":
    ...
    # range_post_cov
    n_iters = 5000
    GEV_knots_trace_stage1 = np.load('GEV_knots_trace_stage1.npy')
    GEV_knots_trace_stage1 = GEV_knots_trace_stage1[-n_iters:, :, :] # use the last n_iters elements
    ...
    if rank == 0:
        GEV_knots_trace_stage2 = np.full(shape=(n_iters, 3, k), fill_value = np.nan) # [n_iters, n_GEV, num_knots]
        # GEV_knots_trace_stage2[0,:,:] = np.tile(np.array([mu, tau, ksi]), (k,1)).T
        GEV_knots_trace_stage2[0,:,:] = GEV_knots_trace_stage1[0,:,:]
        GEV_knots_init = GEV_knots_trace_stage2[0,:,:]
    else:
        GEV_knots_init = None
    ...
    for iter in range(1, n_iters):
        ...
        if rank == 0:
            ...
            if iter % 100 == 0 or iter == n_iters-1:
                ...
                np.save('GEV_knots_trace_stage2', GEV_knots_trace_stage2)
                ...
                GEV_knots_trace_thin = GEV_knots_trace_stage2[0:iter:10,:,:]
        ...
        if iter % 25 == 0:
            ...
            if rank == 0:
                ...
                # num_accepted['GEV'] = 0
                # sample_cov = np.cov(np.array([GEV_knots_trace_stage2[iter-25:iter,0,0].ravel(), # mu location
                #                                 GEV_knots_trace_stage2[iter-25:iter,1,0].ravel()])) # tau scale
    ...
    #### ----- Update GEV mu tau ksi (location, scale, shape) together ----
        ...
        if rank == 0:
            ...
            GEV_knots_proposal = GEV_knots_trace_stage1[iter,:,:]
        else:
        ...
        if rank == 0:
            ...
            # Store the result
            GEV_knots_trace_stage2[iter,:,:] = GEV_knots_update
