...
# load coordinates generated in Python
load('gs_x_ro.gzip')
load('gs_y_ro.gzip')
load('gs_xy_df.gzip')
load('sites_xy_df.gzip')

# Note: if meshgrid is created in python, then matrix(byrow = TRUE) as python
#       changes y first in np.meshgrid(indexing='ij'), but R changes x first
#       in expand.grid
# Example:
#       x <- seq(-102,-92,length.out=10)
#       y <- seq(32, 45, length.out =35)
#       yx <- expand.grid(y,x) # changes y first
#       yx <- data.frame(x = yx$Var2, y = yx$Var1)
#       xy <- data.frame(expand.grid(x,y)) # changes x first
#       colnames(xy) <- c('x','y')
#       gs_xy_df # changes y first

# Note: in printing out a basis, always to matrix(byrow = TRUE)
# Example:
#       basis_yx <- smoothCon(s(x, y, k=12, fx=TRUE), data=yx)[[1]]
#       persp(x, y, matrix(basis_yx$X[,c(1)], nrow = 10, ncol = 35, byrow=TRUE),
#             theta = -30, ticktype = 'detailed',
#             main = 'using observed data', xlab = 'x', ylab = 'y', zlab = 'spline')
#       basis_xy <- smoothCon(s(x, y, k=12, fx=TRUE), data=xy)[[1]]
#       persp(x, y, matrix(basis_yx$X[,c(1)], nrow = 10, ncol = 35, byrow=TRUE),
#             theta = -30, ticktype = 'detailed',
#             main = 'using observed data', xlab = 'x', ylab = 'y', zlab = 'spline')

# Generate and Plot the basis k = 12 for mu0
basis <- smoothCon(s(x, y, k = 12, fx = TRUE), data = gs_xy_df)[[1]]
basis_site <- PredictMat(basis, data = sites_xy_df)

persp(gs_x_ro, gs_y_ro, matrix(basis$X[,c(1)], nrow = 50, ncol = 50, byrow=TRUE),
      zlim=c(-2,2), theta = -30, ticktype = 'detailed',
      main = 'using observed data', xlab = 'x', ylab = 'y', zlab = 'spline')

# Generate and Plot the basis k = 18 for mu1
basis <- smoothCon(s(x, y, k = 18, fx = TRUE), data = gs_xy_df)[[1]]
basis_site <- PredictMat(basis, data = sites_xy_df)

persp(gs_x_ro, gs_y_ro, matrix(basis$X[,c(18)], nrow = 50, ncol = 50, byrow=TRUE),
      zlim=c(-2,2), theta = -30, ticktype = 'detailed',
      main = 'using observed data', xlab = 'x', ylab = 'y', zlab = 'spline')
