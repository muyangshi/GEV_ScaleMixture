...
Purpose
-------
Plot spatial difference surfaces for empirical chi:
    Δχ̂ = χ̂_data - χ̂_model,
where χ̂_data is computed from the observed data (with fitted GEV margins)
and χ̂_model is computed from model realization draws.

The colormap is centered at 0 (white) using a diverging normalization so that
positive/negative differences are visually comparable. See the 
README Pipeline section for mapping outputs to manuscript figure numbers.

Outputs
-------
- Surface_chi_diff_h=<h>.pdf  (one figure per lag h, with panels for u in {0.90, 0.95, 0.99})
"""
# %% imports
