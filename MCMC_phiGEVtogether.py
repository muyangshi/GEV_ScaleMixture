...
if __name__ == "__main__":
    ...
    #### ----- Update phi and GEV (location, scale, shape) together ----- parallelized likelihood calculation across N time
    #### ----- Do not update ksi -----
        # propose phi
        if rank == 0:
        ...
        if phi_out_of_range or Scale_out_of_range or Shape_out_of_range:
        ...
        if rank == 0:
            ...
            prior_phi = np.sum(scipy.stats.beta.logpdf(phi_knots_current, a = 5, b = 5))
            prior_phi_proposal = np.sum(scipy.stats.beta.logpdf(phi_knots_proposal, a = 5, b = 5))

            # for now there is only one set of GEV parameters (constant across all time and space)
            ...
            lik = sum(lik_gathered) + prior_scale + prior_mu + prior_phi
            lik_proposal = sum(lik_proposal_gathered) + prior_scale_proposal + prior_mu_proposal + prior_phi_proposal
        ...
        comm.Barrier() # block for phi GEV updates
