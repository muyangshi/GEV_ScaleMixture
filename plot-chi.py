...
Purpose
-------
Compute and plot moving-window empirical chi surfaces for the real data and for
a model realization drawn under the fitted dependence model. The script also
produces lower/upper bound (LB/UB) surfaces for chi.

Overview
--------
- Data: compute probability integral transforms under fitted GEV margins, 
  then estimate chi(h) over moving spatial windows for chosen lags h and 
  thresholds u; produce LB/UB surfaces from time-wise variability.
- Model: draw X*(s,t) = R(s,t)^{phi(s)} g(Z(s,t)) using posterior-mean dependence
  parameters, then estimate chi surfaces and LB/UB analogs.
...
Notes
-----
This script can be computationally heavy; HPC resources are recommended for
full-scale runs. See `run_all.sh` and the README Pipeline section for mapping
outputs to manuscript figure numbers.
"""
# %% imports
