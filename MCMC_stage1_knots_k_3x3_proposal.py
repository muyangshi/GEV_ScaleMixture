...
# (loc, scale, shape) at each of the k knots
...
if __name__ == "__main__":
    ...
    if rank == 0: 
        ...
        for knot_id in range(k):
            knot = str(knot_id)
            sigma_m_sq['GEV_'+knot] = (2.4**2)/3

        # initialize the proposal covariance matrix
        Sigma_0 = {}
        for knot_id in range(k):
            knot = str(knot_id)
            Sigma_0['GEV_'+knot] = 1e-4 * np.identity(3)

        # initialize the acceptance counter
        num_accepted = {}
        for knot_id in range(k):
            knot = str(knot_id)
            num_accepted['GEV_'+knot] = 0
    ...
    for iter in range(1, n_iters):
        ...
        if rank == 0:
            ...
            if iter % 5000 == 0 or iter == n_iters-1:
                ...
                for i in range(k):
                    ...
                    plt.annotate('knot ' + str(i), xy=(xs_thin2[-1], GEV_knots_trace_thin[:,0,i][-1]))
                ...
                for i in range(k):
                    ...
                    plt.annotate('knot ' + str(i), xy=(xs_thin2[-1], GEV_knots_trace_thin[:,1,i][-1]))
                ...
                for i in range(k):
                    ...
                    plt.annotate('knot ' + str(i), xy=(xs_thin2[-1], GEV_knots_trace_thin[:,2,i][-1]))
        ...
        if iter % 25 == 0:
            ...
            if rank == 0:
                ...
                for knot_id in range(k):
                    knot = str(knot_id)
                    r_hat = num_accepted['GEV_'+knot]/25
                    num_accepted['GEV_'+knot] = 0
                    Sigma_0_hat = np.cov(np.array([GEV_knots_trace_stage1[iter-25:iter,0,knot_id].ravel(), # mu location
                                                    GEV_knots_trace_stage1[iter-25:iter,1,knot_id].ravel(), # tau scale
                                                    GEV_knots_trace_stage1[iter-25:iter,2,knot_id].ravel()])) # ksi shape
                    log_sigma_m_sq_hat = np.log(sigma_m_sq['GEV_'+knot]) + gamma2*(r_hat - r_opt)
                    sigma_m_sq['GEV_'+knot] = np.exp(log_sigma_m_sq_hat)
                    Sigma_0['GEV_'+knot] = Sigma_0['GEV_'+knot] + gamma1*(Sigma_0_hat - Sigma_0['GEV_'+knot])
    ...
    #### ----- Update GEV mu tau ksi (location, scale, shape) together ----
        if rank == 0:
            random_walk_3xk = np.full(shape=(3,k), fill_value=np.nan)
            for knot_id in range(k):
                knot = str(knot_id)
                random_walk_3xk[:,knot_id] = np.sqrt(sigma_m_sq['GEV_'+knot]) * random_generator.multivariate_normal(np.zeros(3), Sigma_0['GEV_'+knot])
        ...
        if rank == 0:
            ...
            if u > ratio: # Reject
                ...
                GEV_accepted = True
                for knot_id in range(k):
                    num_accepted['GEV_'+str(knot_id)] += 1
