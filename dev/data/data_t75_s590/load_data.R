# setwd("~/Desktop/Precip/ForMuyang/GEV_station_analysis")
...
## -- JJA_maxima_imputed: the summertime (June, July, August) maxima from 1950 - 2017 (68 years in total)
...
## mu in 1950
cols <- rev(RColorBrewer::brewer.pal(11, "RdBu"))
ggplot(stations)+ geom_point(aes(x=longitude, y=latitude, color=Mu[,1]))+
  geom_polygon( data = map_data("state"), aes(x = long, y = lat, group = group),
                color = 'gray', fill = NA, linewidth = 0.5) +
  scale_color_gradientn(colours = cols, name = expression(paste(mu, " in 1950"))) +
  coord_cartesian(xlim = c(-102, -92.1), ylim =  c(32.5, 45)) +
  force_panelsizes(rows = unit(4.05, "in"),
                   cols = unit(2.55, "in"))
