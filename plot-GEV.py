...
Purpose
-------
Exploratory marginal analysis. We fit a GEV distribution by MLE at each station
(independently), then smooth the noisy MLEs with a bivariate cubic spline
(`scipy.interpolate.SmoothBivariateSpline`) to reveal large-scale spatial trends.

We compare spline-smoothed surfaces of log(sigma) and xi to (negative) elevation
at station locations. The smoothed log(sigma) surface mirrors elevation, while
xi shows little spatial structure. This supports using elevation as the sole
covariate for log(sigma) and xi in the marginal model.

Output
------
- Fig_elev_logsigma_xi_5panel.pdf
"""
# %%
data_seed = 2345
...
# %%
# --- SmoothBivariateSpline: station-only smoothing for log-sigma and xi ---
import numpy as np
import matplotlib.pyplot as plt
import matplotlib
from matplotlib.gridspec import GridSpec
...
# sites_x, sites_y               # arrays of station longitudes & latitudes (same length)
# logsigma_estimates, ksi_estimates
# state_map

# ------------------------------------------------------------
# 0) config / knobs
s_factor_ls = 1.0   # smoothness scale for log-sigma: s = s_factor_ls * N_obs
s_factor_xi = 0.1   # smoothness scale for xi:       s = s_factor_xi * N_obs  (often noisier -> more smoothing)
kx = ky = 3         # cubic spline in x & y

# Optional fixed map bounds (set use_fixed_bounds=False to auto-fit)
use_fixed_bounds = True
xlim = (-102, -92)
ylim = (32, 44)

# Reusable border style for the state overlay (avoid str numbers!)
BORDER = dict(facecolor='none', edgecolor=(0.3, 0.3, 0.3), linewidth=0.4, zorder=0)
...
# ------------------------------------------------------------
# 1) masks and smoothing params
sites_x = np.asarray(sites_x).ravel()
sites_y = np.asarray(sites_y).ravel()
logsigma_estimates = np.asarray(logsigma_estimates).ravel()
ksi_estimates = np.asarray(ksi_estimates).ravel()

mask_coords = np.isfinite(sites_x) & np.isfinite(sites_y)
mask_ls = mask_coords & np.isfinite(logsigma_estimates)
mask_xi = mask_coords & np.isfinite(ksi_estimates)

n_ls = int(mask_ls.sum())
n_xi = int(mask_xi.sum())
if n_ls < 10 or n_xi < 10:
    raise ValueError("Too few finite points to fit smoothing splines.")

s_ls = s_factor_ls * n_ls
s_xi = s_factor_xi * n_xi

# ------------------------------------------------------------
# 2) fit splines
spl_ls = SmoothBivariateSpline(
    sites_x[mask_ls], sites_y[mask_ls], logsigma_estimates[mask_ls],
    kx=kx, ky=ky, s=s_ls
)
spl_xi = SmoothBivariateSpline(
    sites_x[mask_xi], sites_y[mask_xi], ksi_estimates[mask_xi],
    kx=kx, ky=ky, s=s_xi
)

# ------------------------------------------------------------
# 3) evaluate ONLY at observed station locations
logsigma_smooth_at_sites = spl_ls.ev(sites_x, sites_y)
xi_smooth_at_sites       = spl_xi.ev(sites_x,  sites_y)

# color scales: match raw vs smoothed per parameter for fair comparison
vmin_ls = my_floor(np.nanmin(np.concatenate([logsigma_estimates[mask_ls],
                                             logsigma_smooth_at_sites[mask_ls]])), 1)
vmax_ls = my_ceil (np.nanmax(np.concatenate([logsigma_estimates[mask_ls],
                                             logsigma_smooth_at_sites[mask_ls]])), 1)
norm_ls = matplotlib.colors.TwoSlopeNorm(vcenter=(vmin_ls+vmax_ls)/2, vmin=vmin_ls, vmax=vmax_ls)

vmin_xi = float(np.nanmin(np.concatenate([ksi_estimates[mask_xi],
                                          xi_smooth_at_sites[mask_xi]])))
vmax_xi = float(np.nanmax(np.concatenate([ksi_estimates[mask_xi],
                                          xi_smooth_at_sites[mask_xi]])))
norm_xi = matplotlib.colors.TwoSlopeNorm(vcenter=(vmin_xi+vmax_xi)/2, vmin=vmin_xi, vmax=vmax_xi)

# ------------------------------------------------------------
# 4) plot: raw vs smoothed (stations only), separate colorbars
fig = plt.figure(figsize=(17, 11))
gs  = GridSpec(2, 3, figure=fig, wspace=0.15, hspace=0.18)

ax1 = fig.add_subplot(gs[0, 0])  # log-sigma raw
ax2 = fig.add_subplot(gs[0, 1])  # log-sigma smoothed @ stations
ax3 = fig.add_subplot(gs[1, 0])  # xi raw
ax4 = fig.add_subplot(gs[1, 1])  # xi smoothed @ stations
ax5 = fig.add_subplot(gs[0, 2])  # elevation

# --- raw log-sigma ---
sc1 = ax1.scatter(sites_x[mask_ls], sites_y[mask_ls], s=10,
                  c=logsigma_estimates[mask_ls], cmap='viridis', norm=norm_ls, rasterized=True)
state_map.plot(ax=ax1, **BORDER)
ax1.set_title('log(σ) estimates', fontsize = 20)

# --- smoothed log-sigma (stations only) ---
sc2 = ax2.scatter(sites_x[mask_ls], sites_y[mask_ls], s=10,
                  c=logsigma_smooth_at_sites[mask_ls], cmap='viridis', norm=norm_ls, rasterized=True)
state_map.plot(ax=ax2, **BORDER)
ax2.set_title('log(σ) smoothed', fontsize = 20)

# --- raw xi ---
sc3 = ax3.scatter(sites_x[mask_xi], sites_y[mask_xi], s=10,
                  c=ksi_estimates[mask_xi], cmap='viridis', norm=norm_xi, rasterized=True)
state_map.plot(ax=ax3, **BORDER)
ax3.set_title('ξ estimates', fontsize = 20)

# --- smoothed xi (stations only) ---
sc4 = ax4.scatter(sites_x[mask_xi], sites_y[mask_xi], s=10,
                  c=xi_smooth_at_sites[mask_xi], cmap='viridis', norm=norm_xi, rasterized=True)
state_map.plot(ax=ax4, **BORDER)
ax4.set_title('ξ smoothed', fontsize = 20)

# --- elevation (stations) ---
vmin_e = float(np.nanmin(elevations[mask_coords]))
vmax_e = float(np.nanmax(elevations[mask_coords]))
sc5 = ax5.scatter(sites_x[mask_coords], sites_y[mask_coords], s=10,
                  c=-elevations[mask_coords], cmap='viridis', rasterized=True)
state_map.plot(ax=ax5, **BORDER)
ax5.set_title('Elevation (negative, scaled)', fontsize = 20)

# bounds & aspect
if use_fixed_bounds:
    for ax in (ax1, ax2, ax3, ax4, ax5):
        ax.set_xlim(xlim); ax.set_ylim(ylim)
        ax.set_box_aspect((ylim[1]-ylim[0])/(xlim[1]-xlim[0]))
else:
    xmin, xmax = np.nanmin(sites_x[mask_coords]), np.nanmax(sites_x[mask_coords])
    ymin, ymax = np.nanmin(sites_y[mask_coords]), np.nanmax(sites_y[mask_coords])
    pad_x = 0.03*(xmax-xmin); pad_y = 0.03*(ymax-ymin)
    for ax in (ax1, ax2, ax3, ax4, ax5):
        ax.set_xlim(xmin-pad_x, xmax+pad_x); ax.set_ylim(ymin-pad_y, ymax+pad_y)
        ax.set_box_aspect((ymax-ymin+2*pad_y)/(xmax-xmin+2*pad_x))

for ax in (ax1, ax2, ax3, ax4, ax5):
    ax.tick_params(labelsize=16)

# colorbars
cbar1 = fig.colorbar(sc1, ax=ax1, fraction=0.06, pad=0.02)
cbar2 = fig.colorbar(sc2, ax=ax2, fraction=0.06, pad=0.02)
cbar3 = fig.colorbar(sc3, ax=ax3, fraction=0.06, pad=0.02)
cbar4 = fig.colorbar(sc4, ax=ax4, fraction=0.06, pad=0.02)
cbar5 = fig.colorbar(sc5, ax=ax5, fraction=0.06, pad=0.02)
